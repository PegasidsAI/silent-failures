...
- **[`tool/`](tool/)** — [`effect_check.py`](tool/effect_check.py). Standard library only, no dependencies. `effect_check.py selftest` runs 57 cases. Of the 46 that assert a run outcome, only 12 may come out green; the other 34 must not, and the self-test fails if any of them passes. A suite that only demonstrates that green is reachable demonstrates nothing.
...
- **[`skills/`](skills/) and [`commands/`](commands/)** — the same material packaged as a Claude Code plugin, so that an agent writing checks for your jobs has the traps in front of it instead of inventing a threshold. `/plugin marketplace add PegasidsAI/silent-failures`, then `/plugin install effect-check@silent-failures`. The manifest is re-derived from the files on disk in CI: a manifest naming a script that is not there installs cleanly and fails at first use, which is this archive's subject wearing a different hat.
